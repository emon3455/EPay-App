...
import { cashOut } from '../../store/slices/agentSlice';
import { MainStackParamList } from '../../navigation/types';
import apiClient from '../../services/api.service';

type Props = NativeStackScreenProps<MainStackParamList, 'CashOut'>;
...
export const CashOutScreen: React.FC<Props> = ({ navigation }) => {
  ...
  const [commissionRate, setCommissionRate] = useState<number>(0.018); // Default 1.8%
    ...
    const fetchRates = async () => {
      try {
        ...
        if (response.data.success) {
          setCommissionRate(response.data.data.agentCashOutRate || 0.018);
        }
      } catch (error) {
  ...
  const handleCashOut = async () => {
    if (validateForm() && selectedUser) {
      try {
        await dispatch(cashOut({
          userId: selectedUser._id,
          amount: parseFloat(amount),
        })).unwrap();

        Alert.alert('Success', `Cash-out of ৳${parseFloat(amount).toFixed(2)} completed for ${selectedUser.name}!`, [
          { text: 'OK', onPress: () => navigation.goBack() }
        ]);
      } catch (error: any) {
        Alert.alert('Error', error || 'Cash-out failed');
      }
    }
  };
  ...
  const calculateTotal = () => {
    const amt = parseFloat(amount) || 0;
    return amt + calculateCommission(); // Agent receives amount + commission
  };
        ...
        <ScrollView style={styles.scrollView} showsVerticalScrollIndicator={false}>
            ...
            {amount && parseFloat(amount) > 0 && (
              <View style={styles.feeCard}>
                <View style={styles.feeRow}>
                  <Text style={styles.feeLabel}>Amount from user</Text>
                ...
                <View style={styles.feeRow}>
                  <Text style={styles.feeLabelBold}>Total to credit to agent wallet</Text>
                  <Text style={styles.feeValueBold}>
                    ৳{calculateTotal().toFixed(2)}
                  </Text>
                </View>
                <View style={styles.feeRow}>
                  <Text style={styles.feeLabelSmall}>User will be debited</Text>
          ...
          <View style={styles.infoCard}>
            ...
            <Text style={styles.infoText}>
              Cash-out transfers money from user's wallet to your agent wallet. Commission will be calculated automatically.
            </Text>
          </View>
        </ScrollView>
      </KeyboardAvoidingView>
